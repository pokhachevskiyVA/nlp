...
<div style="font-family:sans-serif;max-width:1000px;margin:16px auto;padding:12px 18px;
  ...
  <h3 style="margin:4px 0">Предсказанные точки 1–4 по RR (нет газа)</h3>
  <p style="margin:4px 0">Точки 1–4 (пунктир «1п…4п») — это <b>предсказание ЧСС порога</b>
     по одной RR-записи (модели обучены на 59 лыжниках с газовым анализом). На кривой RR
     точка ставится там, где ЧСС=60000/RR на нагрузке достигает предсказанного уровня.</p>
  <ul style="line-height:1.6;margin:6px 0">
    <li><b style="color:#1f77b4">Точка 1 — лактатный порог.</b> Ошибка ≈5 уд/мин.<br>
        ЧСС ≈ 116.6 + 5.08·z(cut60) + 3.79·z(cut50) + 1.63·z(Михайлов) − 2.37·z(резерв)</li>
    <li><b style="color:#9467bd">Точка 2 — рН-порог.</b> Ошибка ≈8 уд/мин.<br>
        ЧСС ≈ 136.9 + 7.09·z(cut60) − 1.26·z(cut40) + 1.47·z(Михайлов) + 3.56·z(перелом1)</li>
    <li><b style="color:#d62728">Точка 3 — RCP.</b> Ошибка ≈5 уд/мин.<br>
        ЧСС ≈ 167.3 + 6.90·z(пик ЧСС) + 0.50·z(cut20) + 1.64·z(cut15)</li>
    <li><b style="color:#8c564b">Точка 4 — аэробный лимит.</b> Ошибка ≈8 уд/мин.<br>
        ЧСС ≈ 175.6 + 11.15·z(перелом2) + 3.31·z(пик ЧСС) + 0.86·z(резерв)</li>
  </ul>
  <p style="margin:6px 0;color:#555;font-size:13px">
     z(x) = (x − среднее) / σ (стандартизация по обучающей выборке). Признаки (в уд/мин):
     <b>cutXX</b> — ЧСС в момент падения вариабельности RMSSD до XX% от исходной;
     <b>Михайлов</b> — ЧСС в точке «RMSSD-пол» (исчезновение вариабельности);
     <b>перелом1/перелом2</b> — ЧСС в 1-м/2-м переломе тренда RR (Похачевский);
     <b>пик ЧСС</b> = 60000/мин RR; <b>резерв</b> = пик − покой.</p>
  <p style="margin:6px 0;color:#a33;font-size:13px"><b>Важно:</b> в RR-only режиме нет
     синхронизации по пульсу, поэтому точность ниже, чем при наличии газа — это
     <b>ориентир по тренировочным зонам</b>, а не точный ответ. Чёрная линия — начало
     восстановления (надир RR ≈ аэробный лимит).</p>
</div>
"""
...
def detect_points(df, times, rec_start, return_details=False, load_start=0.0,
    ...
    try:
        ...
        for k in range(len(te)):
            if te[k] > lo4 and vo2c[k] >= P4_FRAC * vpk:
                t4 = float(te[k])
                break
...
def make_rr(rr_path=None, recovery_minutes=None, directory='.', out_dir='.',
    ...
    # (старые кандидатные маркеры VT1?/ВСР↓/изл. убраны — на RR-only графике
    #  оставляем только предсказанные точки 1-4 ниже)
